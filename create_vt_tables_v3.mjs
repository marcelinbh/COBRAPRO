...
// Tentar diferentes connection strings do Supabase com a senha correta
const connectionStrings = [
  // Transaction pooler (port 6543) - recomendado
  `postgresql://postgres.${projectId}:${encodeURIComponent(SUPABASE_DB_PASSWORD)}@aws-0-us-east-1.pooler.supabase.com:6543/postgres`,
  // Session pooler (port 5432)
  `postgresql://postgres.${projectId}:${encodeURIComponent(SUPABASE_DB_PASSWORD)}@aws-0-us-east-1.pooler.supabase.com:5432/postgres`,
  // Outros formatos de região
  `postgresql://postgres.${projectId}:${encodeURIComponent(SUPABASE_DB_PASSWORD)}@aws-0-sa-east-1.pooler.supabase.com:6543/postgres`,
  `postgresql://postgres.${projectId}:${encodeURIComponent(SUPABASE_DB_PASSWORD)}@aws-0-sa-east-1.pooler.supabase.com:5432/postgres`,
  // Direct connection
  `postgresql://postgres:${encodeURIComponent(SUPABASE_DB_PASSWORD)}@db.${projectId}.supabase.co:5432/postgres`,
  // Sem encode
...
for (const cs of connectionStrings) {
  const masked = cs.replace(SUPABASE_DB_PASSWORD, '***').replace(encodeURIComponent(SUPABASE_DB_PASSWORD), '***');
