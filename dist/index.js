...
async function sendWhatsAppMessage(phone, text2) {
  const config = await getEvolutionConfig2();
...
async function gerarMensagemRelatorio() {
  const sb2 = await getSupabaseClientAsync();
  if (!sb2) return "";
  const hoje = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const { data: cfgData } = await sb2.from("configuracoes").select("chave, valor");
  const cfg = {};
  (cfgData || []).forEach((r) => {
    cfg[r.chave] = r.valor;
  });
  const nomeEmpresa = cfg["nomeEmpresa"] || cfg["assinaturaWhatsapp"] || "CobraPro";
  const { data: vencendoHoje } = await sb2.from("parcelas").select("id, valor, status, contratos(clientes(nome, telefone))").eq("data_vencimento", hoje).neq("status", "paga");
  const { data: emAtraso } = await sb2.from("parcelas").select("id, valor, data_vencimento, contratos(clientes(nome, telefone))").eq("status", "atrasada").order("data_vencimento", { ascending: true });
  const { data: contratosAtivos } = await sb2.from("contratos").select("id, valor_principal, status").eq("status", "ativo");
  const { data: clientesAtivos } = await sb2.from("clientes").select("id").eq("status", "ativo");
